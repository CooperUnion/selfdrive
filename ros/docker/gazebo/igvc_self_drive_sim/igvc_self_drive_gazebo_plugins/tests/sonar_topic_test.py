...
class SonarTopicTest(unittest.TestCase):
    ...
    def setUp(self):
        for topic in self.sonar_topics:
            self.sonar_subs[topic] = rospy.Subscriber(
                'sonar/' + topic,
                Range,
    ...
    def tearDown(self):
        ...
        if self.delete_model:
            ...
            ) as e:  # ROS shutdown during timeout
                rospy.logerr(str(e))
    ...
    def sonarTopicTest(self):
        for topic in self.sonar_topics:
            ...
            self.assertTrue(
                self.ranges[topic].header.stamp != rospy.Time(0),
                msg='Range topic [%s] not received' % ('sonar/' + topic),
            )

            self.assertEqual(
                first=self.ranges[topic].header.frame_id,
                second=topic + '_link',
                msg='TwistStamped frame_id [%s] should be [%s]'
                % (self.ranges[topic].header.frame_id, topic + '_link'),
            )
