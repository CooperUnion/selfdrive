...
class JointStateTopicTest(unittest.TestCase):
    ...
    def setUp(self):
        self.sub_joints = rospy.Subscriber(
            self.joint_state_topic, JointState, self.__recvJointStates
        )
    ...
    def jointStateTopicTest(self):
        ...
        self.assertTrue(
            self.joint_states.header.stamp != rospy.Time(0),
            msg='Joint states topic [%s] not received'
        ...
        self.assertTrue(
            collections.Counter(self.joint_states.name)
            == collections.Counter(correct_joints),
            msg='Joint names %s incorrect, should be %s'
            % (str(self.joint_states.name), str(correct_joints)),
        )

        # Make sure position array has the appropriate number of elements
        self.assertTrue(
            len(self.joint_states.position) == len(correct_joints),
            msg='Joint state position array not correct length (len = %d, should be %d)'
            % (len(self.joint_states.position), len(correct_joints)),
        )

        # Make sure velocity array has the appropriate number of elements
        self.assertTrue(
            len(self.joint_states.velocity) == len(correct_joints),
            msg='Joint state velocity array not correct length (len = %d, should be %d)'
            % (len(self.joint_states.velocity), len(correct_joints)),
        )
