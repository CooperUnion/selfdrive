...
class SpawnModelTest(unittest.TestCase):
    ...
    def spawnModelTest(self):
        ...
        if self.pub_tf:
            ...
            self.assertTrue(
                self.__tfLookupWithTimeout('base_footprint', 'world'),
    ...
    def __parseXacro(pub_tf, twist_mode):
        urdf_path = (
            rospkg.RosPack().get_path('igvc_self_drive_description')
            + '/urdf/gem.urdf.xacro'
        )
        ...
        if (
            pub_tf
        ):  # Publish a perfectly accurate transform from world-->footprint
            arg_str += ' pub_tf:=true'
        else:
