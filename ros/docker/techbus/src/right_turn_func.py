...
def main():
    ...
    while not rospy.is_shutdown():
        ...
        if stop_found and datetime.datetime.now() - stop_found_time > datetime.timedelta(seconds = 1.5):
            bus.send('DBW_VelocityCommand', {'DBW_linearVelocity': 0})
        else:
            bus.send('DBW_VelocityCommand', {'DBW_linearVelocity': 1.0})

        if stop_found and datetime.datetime.now() - stop_found_time > datetime.timedelta(seconds = 7):
            break
