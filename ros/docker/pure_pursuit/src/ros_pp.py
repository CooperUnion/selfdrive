...
class Subscriber:
    def __init__(self):
        self.odom = rospy.Subscriber(
            '/encoder_odom', Odometry, self.car_update
        )
        self.path = rospy.Subscriber(
            '/teb_local_planner/local_plan', Path, self.set_waypoints
        )
        self.path = rospy.Subscriber(
            '/move_base_simple/goal', PoseStamped, self.set_goal
        )
        self.car = Car()
        self.xgoal = 0
        self.ygoal = 0
    ...
    def set_waypoints(msg):
        for i in range(0, n_points - 1):
            waypoints[i, 0] = msg.PoseStamped[
                i
            ].pose.position.x  # Sets x coordinate of waypoint
            waypoints[i, 1] = msg.PoseStamped[
                i
            ].pose.position.y  # Sets y coordinate of waypoint
...
class Publisher:
    def __init__(self):
        self.cmd = rospy.Publisher(
            '/pure_pursuit/ppc_cmd', Float32MultiArray, queue_size=2
        )
...
def nearest_point(curr_x, curr_y):
    ...
    dist = np.sqrt(
        ((waypoints[:, 0] - curr_x) ** 2) + ((waypoints[:, 1] - curr_y) ** 2)
    )
...
def steering_angle(lookahead_x, lookahead_y, xpos, ypos, yaw):
    ...
    signed = np.sign(
        math.sin(angle) * (lookahead_x - xpos)
        - math.cos(angle) * (lookahead_y - ypos)
    )
        ...
        while dist < dmax:
            ...
            if dist > decel_dist:
                lookahead_x, lookahead_y = lookahead(
                    subscriber.car.xpos, subscriber.car.ypos
                )
                steer_next = steering_angle(
                    lookahead_x,
                    lookahead_y,
                    subscriber.car.xpos,
                    subscriber.car.ypos,
