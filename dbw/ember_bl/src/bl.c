...
#define CANRX_ISOTP_CALLBACK_NAME_(IDENTITY) \
	CANRX_onRxCallback_UPD_IsoTpTx_##IDENTITY
...
void CANTX_populateTemplate_Status(struct CAN_TMessage_BlStatus * const m)
{
	...
	switch (bl_state) {
		case BL_STATE_INIT:
			s = CAN_T_BLSTATUS_STATE_AWAIT_TRIGGER;
			break;
		case BL_STATE_AWAIT_TRIGGER:
			s = CAN_T_BLSTATUS_STATE_AWAIT_TRIGGER;
			break;
		case BL_STATE_RECV_CHUNK:
			s = CAN_T_BLSTATUS_STATE_RECV_CHUNK;
			break;
		case BL_STATE_CHECK_DESC:
			s = CAN_T_BLSTATUS_STATE_CHECK_DESC;
			break;
		case BL_STATE_COMMIT_CHUNK:
			s = CAN_T_BLSTATUS_STATE_COMMIT_CHUNK;
			break;
		case BL_STATE_FINALIZE:
			s = CAN_T_BLSTATUS_STATE_FINALIZE;
			break;
		case BL_STATE_REBOOT_FW:
			s = CAN_T_BLSTATUS_STATE_REBOOT_FW;
			break;
		case BL_STATE_FAULT:
			s = CAN_T_BLSTATUS_STATE_FAULT;
			break;
		case BL_STATE_RESET:
			s = CAN_T_BLSTATUS_STATE_RESET;
			break;
		default:
			s = CAN_T_BLSTATUS_STATE_RESET;
			break;
...
static void bl_step(void)
{
	...
	if (next_state != bl_state) set_state(next_state);
}
