...
def main():
    ...
    parser.add_argument(
        'module_name', type=str, help="Module name (lowercase)"
    )
    args = parser.parse_args()

    img = open(
        f"../../build/dbw/node_fw/{args.module_name}/firmware.bin", "rb"
    )
    img_raw = img.read()
    ...
    t_msg = can.Message(
        arbitration_id=trigger_msg.frame_id, data=t_data, is_extended_id=False
    )
    ...
    for i in range(0, 100):
        ...
        if msg is not None and msg.arbitration_id == response_msg.frame_id:
            ...
            b_data = trigger_msg.encode(
                {'Begin': 1, 'Filler1': 0, 'Trigger': 1}
            )
            b_msg = can.Message(
                arbitration_id=trigger_msg.frame_id,
                data=b_data,
            ...
            for i in trange(0, len(img_raw), step):
                ...
                u_data = updater_msg.encode(
                    {'Position': i / step, 'Data': raw_data}
                )
                u_msg = can.Message(
                    arbitration_id=updater_msg.frame_id,
                    data=u_data,
                    is_extended_id=False,
                )
            ...
            d_msg = can.Message(
                arbitration_id=done_msg.frame_id,
                data=d_data,
                is_extended_id=False,
            )
            bus.send(d_msg)
