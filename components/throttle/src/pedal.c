...
/*
 * Convert voltage out of 3.3V maximum to a duty cycle
 * (0--(2^PWM_RESOLUTION)-1).
 */
static uint32_t voltage_to_duty_cycle(float32_t v)
{
...
static uint32_t convert_throttle_command(struct throttle_output t, float32_t p)
{
	...
	const float32_t voltage
		= t.low_voltage + (p * (t.high_voltage - t.low_voltage));
...
/*
 * Set the pedal outputs according to the given percentage command
 * (0.00 -- 1.00).
 */
